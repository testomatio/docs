---
title: Read-Only User Role in Testomat.io
description: Learn how to add or manage Read-Only users in Testomat.io to provide stakeholders with view-only access to test cases, reports, and analytics without editing permissions.
url: https://docs.testomat.io/management/company/users-and-permissions/read-only-user
head:
  - tag: meta
    attrs:
      name: keywords
      content: Testomat.io read-only role, read-only user, read-only access, stakeholder access, add read-only user, invite read-only user, manage read-only permissions, grant write access, revoke write access, company roles, view-only access
---
...
![Testomat.io - Grant Write Access Project Level](./images/Grant_write_access_project_level.png)